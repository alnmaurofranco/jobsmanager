...
export const ConstantContext = createContext<IConstantContextState>(
  {} as IConstantContextState
)
...
export const ConstantProvider: React.FC = ({ children }) => {
  ...
  const updateJob = useCallback(
    async ({ id, name, dailyHours, totalHours }: IJobData) => {
      try {
  ...
  const createJob = useCallback(
    async ({ name, dailyHours, totalHours }: IJobData) => {
      try {
        const token = Cookie.get('token')
